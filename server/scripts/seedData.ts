import { users, userProfiles, studyGroups, studyGroupMembers, challenges } from "../../drizzle/schema";
import { eq } from "drizzle-orm";

export async function seedAll(db: any) {
  console.log("Seeding test users, groups, and challenges...");
  ...
  for (const tu of testUsers) {
    ...
    if (existing.length === 0) {
      ...
      await db.insert(userProfiles).values({
        ...
        profilePhoto: `https://api.dicebear.com/7.x/avataaars/svg?seed=${tu.name.replace(" ", "")}`,
      });
    } else {
  ...
  const existingCh = await db.select().from(challenges);
  const existingTitles = new Set(existingCh.map((c: any) => c.title));
  ...
  for (const challenge of ALL_CHALLENGES) {
    if (!existingTitles.has(challenge.title)) {
      try {
        ...
        // Safe swallow duplicates
      }
    }
  }

  console.log(`✅ Seeded ${userIds.length} Users, ${groupNames.length} Groups, and ${added} missing Challenges.`);
}
